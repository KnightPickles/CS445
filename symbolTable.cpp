...
void * SymbolTable::lookupWithinScope(std::string sym) {
    void *data;

    data = stack[depth() - 1]->lookup(sym);
    if (debugFlg) printf("DEBUG(SymbolTable): lookup the symbol \"%s\" and %s.\n", sym.c_str(),
...
int main()
{
    ...
    st.insert("test", (char *)"testy");
    st.enter((std::string )"Second");
    st.insert("delta", (char *)"dog"); 
    st.insertGlobal("echo", (char *)"elk"); 
    char* str = (char*)st.lookup("test");
    if(str == NULL) {
        printf("Not found like it should be.");
    } else {
        printf("Found it, like it shouldn't be.");
    }
